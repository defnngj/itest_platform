...
-->
<template>
  <div id="report-details-dialog">
    <el-dialog :visible.sync="dialogVisible" @close="cancelReport()" width="calc(100vw - 360px)">
      <div slot="title" class="common-title">
        报告详情
      </div>
      <div style="overflow:auto; margin-bottom: 50px;" v-loading='loading'>
        <el-button type="primary" size="small" class="report-button" @click="jtlDownload">下载JTL</el-button>
...
export default {
  ...
  methods: {
    ...
    jtlDownload() {
      this.loading = true
      this.result = ReportApi.downloadJtl(this.reportId).then(resp => {
        const name = 'report_' + this.reportId + '.jtl';
        downloadFile(name, resp, { type: 'application/octet-stream' })
      }).catch(e => {
        const text = e.response.data.text();
        text.then(data => {
          this.$message.error({ message: JSON.parse(data).message || e.message, showClose: true });
        });
      })
        .finally(() => {
          this.loading = false
